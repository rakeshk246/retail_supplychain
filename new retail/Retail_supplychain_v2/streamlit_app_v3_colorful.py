...
from scenarios_colorful import get_colorful_scenarios as get_scenarios
...
def render_agent_cards(model, record):
    ...
    """
    for (icon, name, status, info, thought) in cards:
        color_hex = "#22c55e" if status in ("active", "ok") else "#ef4444" if status == "disrupted" else "#eab308"
        bg_hex = "#14532d" if status in ("active", "ok") else "#7f1d1d" if status == "disrupted" else "#78350f"
        alert_class = ""
        if status == "disrupted":
            alert_class = "animation: pulse-red 2s infinite;"
        
        cards_html += f"""
        <div style="flex:1; box-sizing: border-box; min-width: 150px; background: #1e293b; border: 1px solid #334155; border-top: 4px solid {color_hex}; border-radius: 8px; padding: 16px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="font-weight:bold; font-size:16px; color:white;">{icon} {name}</span>
                <span style="font-size:10px; font-weight:bold; padding:2px 8px; border-radius:12px; background:{bg_hex}; color:{color_hex}; {alert_class}">{status.upper()}</span>
            </div>
            <div style="font-size:12px; color:#94a3b8; font-weight:600; margin-bottom:12px;">{info}</div>
            <div style="background:#0f172a; border-radius:6px; padding:10px; font-size:12px; color:#cbd5e1; font-style:italic; border-left: 2px solid #8b5cf6;">
                " {thought[:100]}{'...' if len(thought)>100 else ''} "
            </div>
        </div>
        """
    cards_html += "</div></body></html>"
    components.html(cards_html, height=140, scrolling=False)
...
def main():
    ...
    st.markdown("### Experience the future of autonomous logistics")

    # Score + top metrics
    kpis = st.session_state.kpi.calculate_kpis()
    score = compute_score(kpis)

    # PULSE DASHBOARD (HTML/CSS)
    path_val = model._last_workflow_path or 'NORMAL'
    path_color = '#22c55e' if path_val == 'NORMAL' else '#eab308' if path_val == 'EMERGENCY' else '#ef4444'
    path_emoji = '🟢' if path_val == 'NORMAL' else '🟡' if path_val == 'EMERGENCY' else '🔴'
    
    stock_val = model.warehouse.inventory
    stock_color = '#22c55e' if stock_val > 200 else '#eab308' if stock_val > 50 else '#ef4444'
    
    fill_rate = min(max(kpis.get('Fill Rate (%)', 100), 0), 100)
    fill_color = '#22c55e' if fill_rate > 90 else '#eab308' if fill_rate > 70 else '#ef4444'
    ...
    .dash-container {{ display: flex; gap: 16px; margin-bottom: 24px; flex-wrap: wrap; }}
    .dash-card {{ flex: 1; min-width: 150px; background: linear-gradient(145deg, #1e293b, #0f172a); border-radius: 16px; padding: 20px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border: 1px solid #334155; position: relative; overflow: hidden; }}
    .dash-val {{ font-size: 36px; font-weight: 800; color: white; margin: 8px 0; letter-spacing: -1px; }}
    .dash-label {{ font-size: 13px; color: #94a3b8; text-transform: uppercase; font-weight: 600; letter-spacing: 1px; display: flex; align-items: center; gap: 6px; }}
    
    .dot-red {{ width: 10px; height: 10px; background: #ef4444; border-radius: 50%; display: inline-block; animation: pulse-red 2s infinite; }}
    .dot-green {{ width: 10px; height: 10px; background: #22c55e; border-radius: 50%; display: inline-block; animation: pulse-green 2s infinite; }}
    .dot-yellow {{ width: 10px; height: 10px; background: #eab308; border-radius: 50%; display: inline-block; animation: pulse-yellow 2s infinite; }}
    
    /* Subtle background glows */
    .glow-stock {{ position: absolute; top: -20px; right: -20px; width: 100px; height: 100px; background: {stock_color}22; border-radius: 50%; filter: blur(20px); }}
    .glow-path {{ position: absolute; top: -20px; right: -20px; width: 100px; height: 100px; background: {path_color}22; border-radius: 50%; filter: blur(20px); }}
    .glow-fill {{ position: absolute; top: -20px; right: -20px; width: 100px; height: 100px; background: {fill_color}22; border-radius: 50%; filter: blur(20px); }}
    </style>
    
    <div class="dash-container">
        <div class="dash-card">
            <div class="glow-score" style="position:absolute; top:-20px; right:-20px; width:100px; height:100px; background:#8b5cf622; border-radius:50%; filter:blur(20px);"></div>
            <div class="dash-label">🏆 Overall Score</div>
            <div class="dash-val" style="color: #a78bfa;">{score}<span style="font-size:18px; color:#64748b;">/100</span></div>
            <div style="font-size:12px; color:#cbd5e1;">Day {model.current_day}</div>
        </div>
        <div class="dash-card">
            <div class="glow-path"></div>
            <div class="dash-label"><div class="{'dot-green' if path_val=='NORMAL' else 'dot-yellow' if path_val=='EMERGENCY' else 'dot-red'}"></div> System Status</div>
            <div class="dash-val">{path_val}</div>
            <div style="font-size:12px; color:#cbd5e1;">Active Routing Path</div>
        </div>
        <div class="dash-card">
            <div class="glow-stock"></div>
            <div class="dash-label">📦 Warehouse Stock</div>
            <div class="dash-val">{stock_val}</div>
            <div style="font-size:12px; color:{stock_color}; font-weight:bold;">{ '✅ Healthy' if stock_val > 200 else '⚠️ Low Stock' if stock_val > 50 else '🚨 Critical Empty' }</div>
        </div>
        <div class="dash-card">
            <div class="glow-fill"></div>
            <div class="dash-label">📈 Customer Fill Rate</div>
            <div class="dash-val">{fill_rate:.0f}%</div>
            <div style="width:100%; height:4px; background:#334155; border-radius:2px; margin-top:8px;">
                <div style="width:{fill_rate}%; height:100%; background:{fill_color}; border-radius:2px;"></div>
            </div>
        </div>
    </div>
    """
    ...
    with tab10:
        st.header("💬 Live Agent Chatroom")
        st.markdown("Watch the AI agents communicate, negotiate, and collaborate in real-time.")

        bus = model.bus if hasattr(model, 'bus') else None
        if bus:
            stats = bus.get_stats()
            c1, c2, c3 = st.columns(3)
            c1.metric("📨 Messages Exchanged", stats.get('total_messages', 0))
            c2.metric("👥 Active Agents", len(stats.get('inbox_sizes', {})))
            c3.metric("📋 Conversation Types", len(stats.get('message_types', [])))
            st.divider()

            # Compile all messages chronologically
            all_messages = []
            if hasattr(bus, 'history'):
                for idx, msg in enumerate(bus.history[-50:]):  # Show last 50
                    all_messages.append({
                        'id': idx,
                ...
                # Fallback if no history attribute but inboxes exist
            ...
            if all_messages:
                # Define Agent Personas and Colors
                agent_styles = {
                    'Demand': {'color': '#3b82f6', 'bg': '#eff6ff', 'align': 'flex-start', 'avatar': '👩‍💼'},
                    'Warehouse': {'color': '#8b5cf6', 'bg': '#f5f3ff', 'align': 'center', 'avatar': '🏗️'},
                    'Supplier': {'color': '#f59e0b', 'bg': '#fffbeb', 'align': 'flex-end', 'avatar': '🏭'},
                    'Logistics': {'color': '#10b981', 'bg': '#ecfdf5', 'align': 'flex-end', 'avatar': '🚚'},
                }
                ...
                /* Agent Specifics */
                .Demand { background: linear-gradient(135deg, #1d4ed8, #3b82f6); border-bottom-left-radius: 2px; }
                .Warehouse { background: linear-gradient(135deg, #6d28d9, #8b5cf6); border-radius: 12px; }
                .Supplier { background: linear-gradient(135deg, #b45309, #f59e0b); border-bottom-right-radius: 2px; }
                .Logistics { background: linear-gradient(135deg, #047857, #10b981); border-bottom-right-radius: 2px; }
                </style>
                ...
                for msg in all_messages:
                    sender = msg['from']
                    style = agent_styles.get(sender, {'align': 'flex-start', 'avatar': '🤖', 'bg': '#475569'})
                    align = 'flex-start' if sender in ['Demand', 'Warehouse'] else 'flex-end'
                    margin = 'margin-right: auto;' if align == 'flex-start' else 'margin-left: auto;'
                    
                    # Highlight urgent words
                    content_raw = msg['content']
                    msg_type = msg['type']
                    msg_type_str = str(msg_type).lower()
                    tag_color = '#ef4444' if 'urgent' in msg_type_str or 'alert' in msg_type_str else '#ffffff33'
                    
                    # Make content human readable instead of raw dicts
                    import ast
                    readable_content = content_raw
                    try:
                        # Try to parse the string representation of a dict back into a dict
                        if content_raw.startswith('{') and content_raw.endswith('}'):
                            content_dict = ast.literal_eval(content_raw)
                            
                            if msg_type == 'DEMAND_FORECAST':
                                pred = content_dict.get('predicted_demand', 0)
                                trend = content_dict.get('trend', 'stable')
                                readable_content = f"I forecast we will need **{pred} units** tomorrow. The trend looks **{trend}**."
                            elif msg_type == 'ORDER_REQUEST':
                                qty = content_dict.get('quantity', 0)
                                prio = content_dict.get('priority', 'normal')
                                readable_content = f"We need to order **{qty} units** immediately! (Priority: {prio.upper()})"
                            elif msg_type == 'SHIPMENT_UPDATE':
                                qty = content_dict.get('quantity', 0)
                                status = content_dict.get('status', 'unknown')
                                readable_content = f"Update: **{qty} units** are currently **{status}**."
                            elif msg_type == 'INVENTORY_ALERT':
                                curr = content_dict.get('current_inventory', 0)
                                status = content_dict.get('status', 'low')
                                readable_content = f"⚠️ Alert! Our inventory is **{status}**! We only have {curr} units left."
                            elif msg_type == 'DISRUPTION_ALERT':
                                dtype = content_dict.get('disruption_type', 'unknown')
                                dur = content_dict.get('duration', 0)
                                readable_content = f"🚨 **CRITICAL DISRUPTION**: We have a {dtype} issue that will last for {dur} days!"
                            else:
                                # Fallback humanization
                                parts = [f"**{k.replace('_', ' ').title()}:** {v}" for k, v in content_dict.items()]
                                readable_content = "<br>".join(parts)
                    except Exception:
                        pass # Keep raw if parsing fails
                    
                    chat_html += f"""
                    <div class="chat-message" style="{margin} align-items: {align};">
                        <div class="chat-header" style="color: #cbd5e1; justify-content: {align};">
                            {style['avatar']} {sender} <span style="color:#64748b; font-weight:normal;">to</span> {msg['to']}
                        </div>
                        <div class="chat-bubble {sender}">
                            <div style="margin-bottom: 6px;">
                                <span class="chat-tag" style="background:{tag_color}">{msg_type}</span>
                            </div>
                            <div style="line-height: 1.5;">{readable_content}</div>
                        </div>
                    </div>
                    """
                
                chat_html += "</div>"
                components.html(chat_html, height=540, scrolling=True)

            else:
                st.info("👋 The chatroom is quiet. Run a few simulation steps to watch the agents start talking!")
        else:
